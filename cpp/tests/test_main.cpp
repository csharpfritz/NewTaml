...
int main()
{
    ...
    std::cout << "DEBUG: About to test Boost.Describe" << std::endl;

    // Test if Boost.Describe integration works
    std::cout << "\n=== Testing Boost.Describe Integration ===" << std::endl;

    // Create an instance
    TestPerson person{"Alice", 30};

    // Test serialization using Boost.Describe
    std::cout << "Testing Boost.Describe serialization..." << std::endl;
    std::cout << "Person name: " << person.name << ", age: " << person.age << std::endl;

    // Test serialization through Serializer
    std::string serializedPerson = Taml::Serializer::Serialize(std::any(person));
    std::cout << "Serialized person:\n"
              << serializedPerson << std::endl;

    // For now, just verify the struct works
    std::cout << "Boost.Describe integration: PASSED (struct defined and accessible)" << std::endl;
